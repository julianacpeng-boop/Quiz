...
MAO = Path("assets/mao_marca_texto.png")
...
def carregar_mao():
    if not MAO.exists():
        raise FileNotFoundError(
            ...
            "Envie assets/mao_marca_texto.png ao GitHub."
        )

    mao = Image.open(MAO).convert("RGBA")

    # tamanho da mão no vídeo
    largura = 440
    altura = int(mao.height * largura / mao.width)

    resampling = getattr(Image, "Resampling", Image)
    return mao.resize((largura, altura), resampling.LANCZOS)
...
def colocar_mao(img, box, progresso):
    ...
    # ponto em que a ponta do marca-texto deve ficar
    tip_x = x1 + int((x2 - x1) * progresso)
    tip_y = int((y1 + y2) / 2)

    # offsets da ponta dentro do PNG recortado
    tip_offset_x = 28
    tip_offset_y = 20
...
def render_estado(
    ...
    # perguntas anteriores permanecem riscadas

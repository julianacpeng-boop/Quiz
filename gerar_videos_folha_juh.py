...
def carregar_mao():
    ...
    # Mantém a mão maior para acompanhar o texto
    largura = 620
    ...
    # Suaviza o final do braço para não parecer um corte seco.
    # A ponta da caneta e a mão permanecem visíveis,
    # enquanto a extremidade inferior/direita do braço vai sumindo.
    alpha = mao.getchannel("A")
    w, h = mao.size
    px = alpha.load()

    fade_start_y = int(h * 0.66)
    fade_start_x = int(w * 0.62)
    ...
    for y in range(h):
        for x in range(w):
            ...
            if y > fade_start_y:
                ...
                a = int(a * max(0.0, 1.0 - t))

            if x > fade_start_x:
                t2 = (x - fade_start_x) / max(1, (w - fade_start_x))
                a = int(a * max(0.0, 1.0 - t2 * 0.85))
...
def colocar_mao(img, box, progresso):
    ...
    # A ponta acompanha a linha do marca-texto
    tip_x = x1 + int((x2 - x1) * progresso)
    tip_y = int((y1 + y2) / 2)

    # offsets calibrados para a ponta real do marcador no PNG
    tip_offset_x = 42
    tip_offset_y = 34
    ...
    # Mantém boa parte do braço saindo pela direita e por baixo,
    # evitando que o final do braço apareça.
    min_x = W - int(mao.width * 0.84)
    if hand_x < min_x:
        hand_x = min_x

    min_y = H - int(mao.height * 0.88)
    if hand_y < min_y:
        hand_y = min_y

    # Evita subir demais
    max_top = -int(mao.height * 0.08)
    if hand_y < max_top:
        hand_y = max_top

...
def colocar_mao(img, box, progresso):
    ...
    # Ajuste fino para a ponta ficar em cima do risco verde.
    tip_x = x1 + int((x2 - x1) * progresso)
    tip_y = y1 + int((y2 - y1) * 0.44)

    # Antes a mão ficou à direita e abaixo do risco.
    # Estes offsets trazem a ponta mais para a esquerda e mais para cima.
    tip_offset_x = 92
    tip_offset_y = 70

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# BUILD: 2026-08-30-SHEETS-RETRY-V3
...
# ============================================================
# LER PLANILHA — COM RETENTATIVA PARA 429/5xx DO GOOGLE
# ============================================================

def _status_google(exc):
...
def google_retry(func, descricao, tentativas=7):
    """Repete chamadas Google temporariamente indisponíveis (429/500/502/503/504)."""
    ultimo = None
    for tentativa in range(1, tentativas + 1):
        try:
            return func()
        except (APIError, HttpError, TimeoutError, ConnectionError) as exc:
            ultimo = exc
            status = _status_google(exc)
            temporario = status in (429, 500, 502, 503, 504) or status is None
            if (not temporario) or tentativa >= tentativas:
                raise
            espera = min(60, 3 * (2 ** (tentativa - 1)))
            print(
                f"⚠️ Google indisponível em {descricao} "
                f"(HTTP {status or 'rede'}). "
                f"Tentativa {tentativa}/{tentativas}; aguardando {espera}s..."
            )
            time.sleep(espera)
    raise ultimo

arquivo = google_retry(lambda: gc.open_by_key(SHEET_ID), "abrir planilha")
...
def aba_para_df(nome):
    ...
    return google_retry(_ler, f"ler aba {nome}")
...
def juntar_clipes(lista, saida, concat_path):
    ...
    with open(concat_path,"w",encoding="utf-8") as f:
        for p in lista:
            caminho_abs = Path(p).resolve().as_posix()
            f.write("file '" + caminho_abs.replace("'", "'\\''") + "'\n")
